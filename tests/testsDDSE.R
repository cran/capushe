...
datapen[6,]=datapen[5,]
datapen[6,4]=1
plot(DDSE(datapen),newwindow=FALSE)
datapen[6,4]=0.005
plot(DDSE(datapen),newwindow=FALSE)

DDSEpartial=DDSE(datapartialcapushe)
validation(DDSEpartial,datavalidcapushe,newwindow=FALSE)

dataNan=datavalidcapushe
dataNan[2,3]=NaN
validation(DDSEpartial,dataNan,newwindow=FALSE)
dataNan[,3]=rep(NaN,3)
validation(DDSEpartial,dataNan,newwindow=FALSE)
